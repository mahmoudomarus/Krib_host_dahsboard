#!/usr/bin/env python3
"""
Create New Confirmed Users and Properties
Since email verification is now disabled, create fresh users and properties
"""
...
# New test users (different emails to avoid conflicts)
NEW_TEST_USERS = [
...
# Dubai property data (smaller set for quick testing)
DUBAI_AREAS = [
...
PROPERTY_TYPES = ["studio", "apartment", "villa", "house", "condo", "cabin", "other"]
...
def create_new_user(user_data):
    """Create a new user with email verification disabled"""
...
def create_property_for_user(user_data, access_token, property_number):
    ...
    base_prices = {
        ...
        "house": random.randint(800, 2000),
        "condo": random.randint(600, 1500),
        "cabin": random.randint(400, 800),
        "other": random.randint(300, 1000)
    }
...
def main():
    print("🏠 KRIB AI - Create New Confirmed Users & Properties")
    print("=" * 60)
    print("🎯 Goal: Create fresh users (with verification disabled) + properties")
    print("👥 Users: 10 new host accounts")
    print("🏠 Properties: 5 properties per user (50 total for quick test)")
    print("🔑 Method: Backend API with fresh authentication")
    print("=" * 60)
    ...
    # Step 1: Create 10 new users
    print(f"\n2️⃣ Creating 10 new users...")
    for i, user_data in enumerate(NEW_TEST_USERS):
        print(f"\nUser {i+1}/10:")
    ...
    print(f"\n✅ Successfully created {len(all_users)} new users")
    
    if not all_users:
        print("❌ No users created successfully - stopping")
        return
    
    # Step 2: Login each user and create 5 properties (for quick test)
    print(f"\n3️⃣ Creating properties for each user...")
    for user_index, user in enumerate(all_users):
        ...
        # Create 5 properties (smaller number for quick test)
        print(f"  Creating 5 properties...")
        user_properties = []
        for prop_num in range(5):
        ...
        print(f"  ✅ Created {len(user_properties)}/5 properties for {user['name']}")
    ...
    # Summary
    print(f"\n🎉 USER & PROPERTY CREATION COMPLETE!")
    print(f"=" * 50)
    print(f"👥 New Users Created: {len(all_users)}/10")
    print(f"🏠 Total Properties: {len(all_properties)}")
    print(f"🌐 API Endpoint: {RENDER_API_BASE}")
    print(f"🔑 API Key: {API_KEY}")
    
    print(f"\n📧 Created Users (confirmed & ready):")
