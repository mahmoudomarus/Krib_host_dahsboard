...
async def send_host_notification(
    ...
    try:
        ...
        return ExternalAPIResponse(
            success=True, data=result, message="Notification sent successfully"
        )
...
async def get_host_notifications(
    ...
    try:
        ...
        return ExternalAPIResponse(success=True, data=result)
...
@router.put(
    "/v1/hosts/{host_id}/notifications/{notification_id}/read",
    response_model=ExternalAPIResponse,
)
async def mark_notification_read(
    host_id: str, notification_id: str, current_user: dict = Depends(get_current_user)
):
...
@router.put(
    "/v1/hosts/{host_id}/notifications/read-all", response_model=ExternalAPIResponse
)
async def mark_all_notifications_read(
    host_id: str, current_user: dict = Depends(get_current_user)
):
...
@router.delete(
    "/v1/hosts/{host_id}/notifications/{notification_id}",
    response_model=ExternalAPIResponse,
)
...
@router.get(
    "/v1/hosts/{host_id}/notifications/count", response_model=ExternalAPIResponse
)
async def get_notification_count(
    ...
    try:
        ...
        return ExternalAPIResponse(
            success=True, data={"host_id": host_id, "unread_count": unread_count}
        )
...
@router.get(
    "/v1/hosts/{host_id}/notifications/statistics", response_model=ExternalAPIResponse
)
...
@router.post(
    "/v1/external/hosts/{host_id}/booking-notifications",
    response_model=ExternalAPIResponse,
)
async def create_booking_notification(
    ...
    try:
        ...
        booking_result = (
            supabase_client.table("bookings").select("*").eq("id", booking_id).execute()
        )
    ...
    try:
        ...
        for notification_data in notifications:
            try:
                ...
                result = await NotificationService.send_host_notification(
                    host_id, notification
                )
                results.append(
                    {
                        "host_id": host_id,
