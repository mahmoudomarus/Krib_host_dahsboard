...
async def get_financial_summary(
    ...
    try:
        ...
        payouts_result = (
            supabase_client.table("host_payouts")
            .select("*")
            .eq("user_id", user_id)
...
async def add_bank_account(
    bank_account: BankAccountCreate, current_user: dict = Depends(get_current_user)
):
...
async def request_payout(
    payout_request: PayoutRequest, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        bank_result = (
            supabase_client.table("host_bank_accounts")
            .select("*")
            .eq("id", payout_request.bank_account_id)
            .eq("user_id", user_id)
            .execute()
        )
...
async def get_payouts(
    limit: int = Query(20, le=100), current_user: dict = Depends(get_current_user)
):
    """Get user's payout history"""
    try:
        result = (
            supabase_client.table("host_payouts")
            .select("*, host_bank_accounts(bank_name, account_number_last4)")
            .eq("user_id", current_user["id"])
            .order("created_at", desc=True)
            .limit(limit)
            .execute()
...
async def update_payout_settings(
    settings: PayoutSettingsUpdate, current_user: dict = Depends(get_current_user)
):
...
def _calculate_next_payout_date(user_id: str) -> Optional[str]:
    ...
    try:
        ...
        if (
            not settings_result.data
            or not settings_result.data[0]["auto_payout_enabled"]
        ):
            return None
        ...
        if frequency == "weekly":
            days_ahead = (
                settings.get("payout_day_of_week", 5) - now.weekday()
            )  # Default Friday
            if days_ahead <= 0:
                days_ahead += 7
