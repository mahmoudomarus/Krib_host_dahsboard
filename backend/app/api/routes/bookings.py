...
async def create_booking(
    ...
    try:
        ...
        property_info = query_service.get_active_property_by_id(
            booking_data.property_id
        )

        # Validate guest count
        query_service.validate_guest_count(
            booking_data.guests, property_info["max_guests"]
        )
        ...
        has_conflict = query_service.check_booking_conflicts(
            booking_data.property_id, booking_data.check_in, booking_data.check_out
        )
    ...
    try:
        ...
        if "check_in" in update_data or "check_out" in update_data:
            check_in = update_data.get(
                "check_in",
                datetime.strptime(existing_booking["check_in"], "%Y-%m-%d").date(),
            )
            check_out = update_data.get(
                "check_out",
                datetime.strptime(existing_booking["check_out"], "%Y-%m-%d").date(),
            )
            ...
            property_result = (
                ...
                .eq("id", existing_booking["property_id"])
                .execute()
            )
            if property_result.data:
        ...
        property_result = (
            supabase_client.table("properties")
            .select("title, address")
            .eq("id", updated_booking["property_id"])
            .execute()
        )
        if property_result.data:
...
async def confirm_booking(
    ...
    try:
        ...
        supabase_client.table("properties").update(
            {
                "booking_count": supabase_client.table("properties")
                .select("booking_count")
                .eq("id", booking["property_id"])
                .execute()
                .data[0]["booking_count"]
                + 1,
                "total_revenue": supabase_client.table("properties")
        ...
        property_result = (
            supabase_client.table("properties")
            .select("title, address")
            .eq("id", confirmed_booking["property_id"])
            .execute()
        )
        if property_result.data:
...
async def cancel_booking(
    ...
    try:
        ...
        property_result = (
            supabase_client.table("properties")
            .select("title, address")
            .eq("id", cancelled_booking["property_id"])
            .execute()
        )
        if property_result.data:
