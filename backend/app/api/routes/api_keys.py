...
class CreateAPIKeyRequest(BaseModel):
    ...
    tier: str = Field(
        "standard", description="Permission tier: read_only, standard, full_access"
    )
    permissions: Optional[List[str]] = Field(
        None, description="Custom permissions (overrides tier)"
    )
...
class CreateAPIKeyResponse(BaseModel):
    ...
    api_key: str = Field(
        ..., description="The API key - SAVE THIS NOW, it won't be shown again!"
    )
...
async def generate_api_key(
    request: CreateAPIKeyRequest, current_user: dict = Depends(verify_admin_user)
):
    """
    ...
    try:
        ...
        logger.info(
            f"API key generated by user {current_user['id']}: {key_info['key_prefix']}..."
        )
...
async def list_api_keys(
    include_revoked: bool = False, current_user: dict = Depends(verify_admin_user)
):
    """
    List all API keys
    ...
    try:
        keys = api_key_service.list_keys(
            include_revoked=include_revoked, created_by=None  # Admins can see all keys
        )
...
async def get_api_key(key_id: str, current_user: dict = Depends(verify_admin_user)):
...
async def revoke_api_key(key_id: str, current_user: dict = Depends(verify_admin_user)):
    """
    Revoke an API key
    ...
    try:
        success = api_key_service.revoke_key(
            key_id=key_id, revoked_by=current_user["id"]
        )
...
async def get_api_key_usage(
    key_id: str, days: int = 30, current_user: dict = Depends(verify_admin_user)
):
...
async def list_all_permissions(current_user: dict = Depends(verify_admin_user)):
