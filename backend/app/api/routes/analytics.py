...
async def get_market_comparison(
    ...
    try:
        ...
        benchmarks = dubai_market_service.get_market_benchmarks(
            area=area, property_type=property_data.get("property_type", "apartment")
        )
        ...
        comparison = {
            ...
            "variance_percent": (
                round(((user_rate - market_rate) / market_rate * 100), 1)
                if market_rate > 0
                else 0
            ),
            "position": (
                ...
                else "Below Market" if user_rate < market_rate else "At Market"
            ),
        }
...
async def get_analytics(
    ...
    try:
        ...
        monthly_growth, booking_growth, rating_change = _calculate_growth_metrics(
            properties, bookings
        )
        ...
        # Cache the results
        await cache_service.cache_analytics_data(
            user_id, period, analytics_response.dict()
        )
...
async def get_dashboard_overview(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        monthly_bookings = len(
            [
                b
                for b in current_month_bookings.data
                if b["status"] in ["confirmed", "completed"]
            ]
        )
        monthly_revenue = sum(
...
def _empty_analytics_response() -> AnalyticsResponse:
    ...
    return AnalyticsResponse(
        ...
        forecast={"next_quarter_revenue": 0, "confidence": 0, "peak_period": None},
        recommendations=[],
    )
...
def _calculate_growth_metrics(
    ...
    try:
        ...
        current_bookings = [
            b
            for b in bookings
            if datetime.fromisoformat(b["created_at"].replace("Z", "+00:00")).replace(
                tzinfo=None
            )
            >= current_month
        ...
        previous_bookings = [
            ...
            >= previous_month
            and datetime.fromisoformat(b["created_at"].replace("Z", "+00:00")).replace(
                tzinfo=None
            )
            < current_month
            and b["status"] in ["confirmed", "completed"]
        ...
        booking_growth = (
            (
                (len(current_bookings) - len(previous_bookings))
...
def _calculate_property_occupancy_rate(
    property_data: Dict, bookings: List[Dict]
) -> float:
...
def _generate_property_performance(
    properties: List[Dict], bookings: List[Dict]
) -> List[Dict[str, Any]]:
    """Generate property performance data"""
    performance_data = []
        ...
        performance_data.append(
            {
                ...
                "occupancy_rate": _calculate_property_occupancy_rate(
                    property_data, property_bookings
                ),
            }
        )
...
def _generate_dubai_forecast(
    properties: List[Dict], total_revenue: float
) -> Dict[str, Any]:
    ...
    for month_data in market_forecast["forecast_data"]:
        user_forecasted_revenue = (
            user_monthly_baseline * month_data["seasonal_multiplier"]
        )
...
def _generate_recommendations(
    properties: List[Dict], bookings: List[Dict], total_revenue: float
) -> List[Dict[str, Any]]:
        ...
        formatted_bookings.append(
            {
                ...
                "property_name": property_map.get(
                    booking["property_id"], "Unknown Property"
                ),
...
def _generate_pricing_insights(
    ...
    avg_rate = sum(
        float(b["total_amount"]) / int(b["nights"]) for b in confirmed_bookings
