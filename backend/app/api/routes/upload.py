...
async def upload_property_images(
    ...
    try:
        ...
        # Update property
        supabase_client.table("properties").update({"images": updated_images}).eq(
            "id", property_id
        ).execute()
...
@router.post(
    "/property/{property_id}/presigned-upload", response_model=PresignedUploadResponse
)
...
@router.delete(
    "/property/{property_id}/images/{s3_key:path}", response_model=SuccessResponse
)
async def delete_property_image(
    property_id: str, s3_key: str, current_user: dict = Depends(get_current_user)
):
...
async def reorder_property_images(
    ...
    try:
        ...
        current_images = property_data.get("images", [])
        if len(image_urls) != len(current_images) or set(image_urls) != set(
            current_images
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        supabase_client.table("properties").update({"images": image_urls}).eq(
            "id", property_id
        ).execute()
