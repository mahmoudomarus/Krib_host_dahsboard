...
@router.post("/v1/bookings/{booking_id}/process-payment", response_model=ExternalAPIResponse)
...
async def process_booking_payment(
    ...
    try:
        ...
        logger.info(f"Processing payment for booking {booking_id}, amount: {booking['total_amount']} AED")
        ...
        try:
            ...
            return ExternalAPIResponse(
                ...
                data={
                    ...
                    "host_payout": {
                        ...
                        "platform_fee_percentage": payout_amounts["platform_fee_percentage"]
                    },
            ...
            logger.error(f"Stripe payment error for booking {booking_id}: {str(stripe_error)}")
...
@router.get("/v1/bookings/{booking_id}/payment-status", response_model=ExternalAPIResponse)
...
async def get_payment_status(
    ...
    try:
        booking_result = supabase_client.table("bookings").select(
            "id, status, payment_status, total_amount, payment_method"
        ).eq("id", booking_id).execute()
