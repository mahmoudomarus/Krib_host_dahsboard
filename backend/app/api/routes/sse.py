...
class SSEConnectionManager:
    ...
    def add_connection(self, host_id: str) -> asyncio.Queue:
        ...
        logger.info(
            f"SSE connection added for host {host_id}. Total connections: {self._connection_count}"
        )
    ...
    def remove_connection(self, host_id: str, connection_queue: asyncio.Queue):
        ...
        logger.info(
            f"SSE connection removed for host {host_id}. Total connections: {self._connection_count}"
        )
...
async def event_stream(
    host_id: str, connection_queue: asyncio.Queue
    ...
    try:
        ...
        data_check_task = asyncio.create_task(
            check_for_updates(host_id, connection_queue)
        )
            ...
            while True:
                ...
                try:
                    event_data = await asyncio.wait_for(
                        connection_queue.get(), timeout=1.0
                    )
        ...
        while True:
            ...
            try:
                ...
                for notification in notifications:
                    event_data = {"type": "notification", "data": notification}
                    try:
                        connection_queue.put_nowait(event_data)
                    except asyncio.QueueFull:
                        break
                ...
                for update in booking_updates:
                    event_data = {"type": "booking_update", "data": update}
                    try:
                        connection_queue.put_nowait(event_data)
                    except asyncio.QueueFull:
                        break
...
async def host_event_stream(
    host_id: str, request: Request, current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if current_user["id"] != host_id:
            raise HTTPException(
                status_code=403,
                detail="Access denied - you can only access your own event stream",
            )
...
async def send_custom_event(
    ...
    try:
        ...
        if current_user["id"] != host_id:
            raise HTTPException(
                status_code=403,
                detail="Access denied - you can only send events to your own stream",
            )
        ...
        logger.info(
            f"Custom event sent to host {host_id}: {event_data.get('type', 'unknown')}"
        )
...
async def get_sse_statistics(current_user: dict = Depends(get_current_user)):
...
async def send_sse_notification(host_id: str, notification: Dict[str, Any]):
    ...
    try:
        ...
        logger.debug(
            f"SSE notification sent to host {host_id}: {notification.get('title', 'unknown')}"
        )
...
async def send_sse_booking_update(host_id: str, booking_update: Dict[str, Any]):
    ...
    try:
        ...
        logger.debug(
            f"SSE booking update sent to host {host_id}: {booking_update.get('booking_id', 'unknown')}"
        )
