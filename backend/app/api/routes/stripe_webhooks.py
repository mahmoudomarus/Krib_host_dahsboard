...
async def handle_stripe_webhook(
    ...
    try:
        ...
        if StripeConfig.STRIPE_WEBHOOK_SECRET:
            ...
            logger.warning("Processing webhook without signature verification (no webhook secret configured)")
        ...
        payment_intent_id = event_data.get("payment_intent") if "payment_intent" in event_data else event_data.get("id") if event_type.startswith("payment_intent") else None
        charge_id = event_data.get("charge") if "charge" in event_data else event_data.get("id") if event_type.startswith("charge") else None
        transfer_id = event_data.get("transfer") if "transfer" in event_data else event_data.get("id") if event_type.startswith("transfer") else None
        payout_id = event_data.get("payout") if "payout" in event_data else event_data.get("id") if event_type.startswith("payout") else None
...
async def process_webhook_event(event_id: str, event_type: str, event_data: dict):
    ...
    if event_type == "account.updated":
        ...
        await stripe_connect_service.handle_account_updated_webhook(account_id, event_data)
    
    # Payment Intent events
    elif event_type == "payment_intent.succeeded":
        payment_intent_id = event_data.get("id")
        await stripe_payment_service.handle_payment_intent_succeeded(payment_intent_id, event_data)
...
async def handle_payment_failed(payment_intent_id: str, payment_intent_data: dict):
    ...
    try:
        # Find booking
        booking_result = supabase_client.table("bookings").select("id").eq(
...
async def handle_charge_refunded(charge_data: dict):
    ...
    try:
        ...
        amount_refunded = charge_data.get("amount_refunded", 0) / 100  # Convert from fils
        
        # Find booking
        booking_result = supabase_client.table("bookings").select("id, total_amount").eq(
            "stripe_charge_id", charge_id
        ).execute()
        ...
        if booking_result.data:
            ...
            status = "refunded" if refunded or amount_refunded >= total else "partially_refunded"
            
            supabase_client.table("bookings").update({
                "payment_status": status,
                "refund_amount": amount_refunded,
                "status": "cancelled" if status == "refunded" else booking.get("status"),
                "updated_at": datetime.utcnow().isoformat()
...
async def handle_payout_paid(payout_data: dict):
    ...
    try:
        ...
        logger.info(f"Stripe payout {payout_id} paid to bank, arrival date: {arrival_date}")
    except Exception as e:
