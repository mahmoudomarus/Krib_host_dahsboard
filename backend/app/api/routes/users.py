...
async def update_user_profile(
    user_updates: UserUpdate, current_user: dict = Depends(get_current_user)
):
    """Update user profile"""
    try:
...
async def update_user_settings(
    settings: Dict[str, Any], current_user: dict = Depends(get_current_user)
):
...
async def change_password(
    ...
    try:
        ...
        has_email_provider = any(
            identity.provider == "email" for identity in user.identities
        )
        has_oauth_provider = any(
            identity.provider in ["google", "github", "facebook"]
...
async def get_user_notifications(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        notifications = settings.get(
            "notifications",
            {"bookings": True, "marketing": False, "system_updates": True},
        )
...
async def update_user_notifications(
    notifications: Dict[str, bool], current_user: dict = Depends(get_current_user)
):
