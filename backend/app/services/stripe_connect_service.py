...
class StripeConnectService:
    ...
    async def create_connect_account(
        ...
        try:
            ...
            user_result = supabase_client.table("users").select("stripe_account_id, email, name").eq("id", user_id).execute()
            ...
            if user.get("stripe_account_id"):
                logger.info(f"User {user_id} already has Stripe account: {user['stripe_account_id']}")
    ...
    async def create_account_link(
        ...
        try:
            ...
            user_result = supabase_client.table("users").select("stripe_account_id").eq("id", user_id).execute()
            
            if not user_result.data or not user_result.data[0].get("stripe_account_id"):
                raise ValueError(f"User {user_id} does not have a Stripe account. Create account first.")
    ...
    async def get_account_status(user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            supabase_client.table("users").update({
                ...
                "stripe_payouts_enabled": account.payouts_enabled,
                "stripe_details_submitted": account.details_submitted,
                "stripe_onboarding_completed": account.charges_enabled and account.payouts_enabled,
    ...
    @staticmethod
    async def handle_account_updated_webhook(account_id: str, account_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            user_result = supabase_client.table("users").select("id").eq("stripe_account_id", account_id).execute()
            ...
            logger.error(f"Error handling account.updated webhook for {account_id}: {e}")
            raise
