...
def generate_daily_analytics_report(date_str: str):
    ...
    try:
        ...
        users_result = (
            supabase_client.table("users").select("id, email, name").execute()
        )
        ...
        for user in users:
            try:
                ...
                user_analytics = _calculate_user_daily_analytics(
                    user["id"], report_date
                )
                ...
                supabase_client.table("analytics_reports").insert(
                    analytics_data
                ).execute()
                reports_generated += 1
                ...
                logger.error(
                    f"Failed to generate analytics for user {user['id']}: {user_error}"
                )
                continue

        logger.info(
            f"Daily analytics report completed: {reports_generated} reports generated"
        )
...
def process_automatic_payout(self, user_id: str):
    ...
    try:
        ...
        settings_result = (
            supabase_client.table("payout_settings")
            .select("*")
            .eq("user_id", user_id)
            .execute()
        )
        ...
        bank_result = (
            supabase_client.table("host_bank_accounts")
            .select("*")
            .eq("user_id", user_id)
            .eq("is_primary", True)
            .execute()
        ...
        payout_result = (
            supabase_client.table("host_payouts").insert(payout_data).execute()
        )
        payout = payout_result.data[0]

        # Send notification email
        send_payout_notification_email.delay(
            user_id,
            {
        ...
        logger.info(
            f"Automatic payout processed for user {user_id}: ${available_balance}"
        )
...
def optimize_property_images(property_id: str, image_urls: List[str]):
    ...
    try:
        ...
        for image_url in image_urls:
            try:
                ...
                optimized_images.append(
                    {"original": image_url, "variants": image_variants}
                )
        ...
        logger.info(
            f"Image optimization completed for property {property_id}: {len(optimized_images)} images processed"
        )
...
# Helper functions
def _calculate_user_daily_analytics(
    ...
    try:
        ...
        bookings_result = (
            ...
            .lt("created_at", (date + timedelta(days=1)).isoformat())
            .execute()
        )
...
@monitored_task(bind=True, max_retries=3)
def send_booking_webhook(
    self, event_type: str, booking_id: str, booking_data: Dict[str, Any]
):
...
@monitored_task(bind=True, max_retries=3)
def send_host_response_webhook(
    self, booking_id: str, host_id: str, response_data: Dict[str, Any]
):
...
def send_host_notification_task(self, host_id: str, notification_data: Dict[str, Any]):
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                NotificationService.send_host_notification(host_id, notification)
            )
        finally:
            loop.close()
...
def cleanup_webhook_and_notifications():
    ...
    try:
        ...
        asyncio.set_event_loop(loop)
        try:
            notification_cleanup = loop.run_until_complete(
        ...
        webhook_cleanup_result = supabase_client.rpc(
            "disable_failed_webhook_subscriptions"
        ).execute()
        disabled_webhooks = webhook_cleanup_result.data or 0

        logger.info(
            f"Cleanup completed: {notification_cleanup['deleted_count']} expired notifications, {disabled_webhooks} failed webhooks disabled"
        )
