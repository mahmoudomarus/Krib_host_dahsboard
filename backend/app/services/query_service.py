...
class QueryService:
    ...
    @staticmethod
    def get_property_by_id(property_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        try:
            query = supabase_client.table("properties").select("*").eq("id", property_id)
    ...
    def get_user_properties(
        ...
        """
        try:
            query = supabase_client.table("properties").select("*").eq("user_id", user_id)
    ...
    def check_booking_conflicts(
        ...
        """
        try:
            query = supabase_client.table("bookings").select("check_in, check_out").eq("property_id", property_id).in_("status", ["confirmed", "pending"])
            ...
            for booking in result.data:
                existing_checkin = datetime.strptime(booking["check_in"], "%Y-%m-%d").date()
                existing_checkout = datetime.strptime(booking["check_out"], "%Y-%m-%d").date()
            ...
            logger.error(f"Error checking booking conflicts for property {property_id}: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    @staticmethod
    def calculate_booking_total(price_per_night: float, check_in: date, check_out: date) -> tuple[int, float]:
        """
    ...
    @staticmethod
    def get_booking_by_id(booking_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        """
        try:
            result = supabase_client.table("bookings").select("*, properties(user_id, title, address)").eq("id", booking_id).execute()
