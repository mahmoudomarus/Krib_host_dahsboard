...
class CacheService:
    ...
    async def cache_user_profile(user_id: str, profile_data: Dict[str, Any]) -> bool:
        ...
        return await redis_client.set(
            cache_key, profile_data, expire=CacheService.CACHE_TIMES["user_profile"]
        )
    ...
    @staticmethod
    async def cache_user_properties(
        user_id: str, properties: List[Dict[str, Any]]
    ) -> bool:
        """Cache user's properties list"""
        cache_key = get_user_cache_key(user_id, "properties")
        return await redis_client.set(
            cache_key, properties, expire=CacheService.CACHE_TIMES["properties"]
        )
    ...
    async def cache_property_details(
        property_id: str, property_data: Dict[str, Any]
    ) -> bool:
    ...
    async def cache_analytics_data(
        user_id: str, period: str, analytics_data: Dict[str, Any]
    ) -> bool:
        """Cache analytics data for user"""
        cache_key = get_user_cache_key(user_id, "analytics", period)
        return await redis_client.set(
            cache_key, analytics_data, expire=CacheService.CACHE_TIMES["analytics"]
        )
    ...
    async def cache_market_data(location: str, market_data: Dict[str, Any]) -> bool:
        ...
        return await redis_client.set(
            cache_key, market_data, expire=CacheService.CACHE_TIMES["market_data"]
        )
    ...
    @staticmethod
    async def cache_financial_summary(
        user_id: str, period: str, summary_data: Dict[str, Any]
    ) -> bool:
    ...
    async def cache_search_results(
        search_params: Dict[str, Any], results: List[Dict[str, Any]]
    ) -> bool:
        ...
        return await redis_client.set(
            cache_key, results, expire=CacheService.CACHE_TIMES["search_results"]
        )
