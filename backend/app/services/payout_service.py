...
class PayoutService:
    ...
    async def process_booking_payout(
        ...
        try:
            ...
            if booking["payment_status"] != "succeeded":
                raise ValueError(f"Cannot process payout - payment status is {booking['payment_status']}")
            ...
            checkout_date = datetime.fromisoformat(booking["check_out"]).date() if isinstance(booking["check_out"], str) else booking["check_out"]
            payout_eligible_date = checkout_date + timedelta(days=StripeConfig.PAYOUT_DELAY_DAYS)
            
            if date.today() < payout_eligible_date:
                raise ValueError(f"Payout not yet eligible. Available after {payout_eligible_date}")
            
            # Calculate amounts
            total_amount = Decimal(str(booking["total_amount"]))
            
            if platform_fee_percentage is None:
                platform_fee_percentage = Decimal(str(StripeConfig.PLATFORM_FEE_PERCENTAGE))
            ...
            logger.info(f"Created transfer {transfer.id} for booking {booking_id}, amount: {host_payout_amount}")
            
            # Create payout record
            payout_result = supabase_client.table("payouts").insert({
            ...
            logger.error(f"Stripe error processing payout for booking {booking_id}: {e}")
    ...
    async def get_host_payouts(
        ...
        try:
            # Build query
            query = supabase_client.table("payouts").select(
            ...
            totals_result = supabase_client.table("payouts").select(
                "status, amount"
            ).eq("user_id", user_id).execute()
            
            # Calculate totals
            total_paid = sum(
                Decimal(str(p["amount"])) for p in totals_result.data 
                if p["status"] == "paid"
            )
            total_pending = sum(
                Decimal(str(p["amount"])) for p in totals_result.data 
            ...
            for payout in payouts_result.data:
                ...
                payouts.append({
                    ...
                    "platform_fee": Decimal(str(payout["platform_fee"])) if payout["platform_fee"] else None,
                    "original_booking_amount": Decimal(str(payout["original_booking_amount"])) if payout["original_booking_amount"] else None,
                    ...
                    "booking_dates": f"{booking_data.get('check_in')} to {booking_data.get('check_out')}" if booking_data.get("check_in") else None
                })
    ...
    @staticmethod
    async def handle_transfer_paid_webhook(transfer_id: str, transfer_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            payout_result = supabase_client.table("payouts").select(
            ...
            logger.info(f"Marked payout {payout['id']} as paid for transfer {transfer_id}")
    ...
    @staticmethod
    async def handle_transfer_failed_webhook(transfer_id: str, transfer_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            logger.error(f"Payout {payout['id']} failed for transfer {transfer_id}: {failure_message}")
            
        except Exception as e:
            logger.error(f"Error handling transfer.failed webhook for {transfer_id}: {e}")
            raise
