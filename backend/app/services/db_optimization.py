...
class DatabaseOptimizer:
    ...
    @staticmethod
    async def get_optimized_user_properties(
    ...
    @staticmethod
    async def get_optimized_recent_bookings(
    ...
    @staticmethod
    async def bulk_update_property_metrics(
        ...
        async with DatabaseOptimizer.query_timer("bulk_update", "property_metrics"):
            try:
                ...
                for metrics_data in property_metrics:
                    ...
                    supabase_client.table("properties").update(metrics_data).eq(
                        "id", property_id
                    ).execute()
    ...
    @staticmethod
    async def get_analytics_with_aggregation(
        user_id: str, period_days: int = 30
    ) -> Dict[str, Any]:
        ...
        async with DatabaseOptimizer.query_timer("aggregation", "analytics"):
            try:
                ...
                bookings_result = (
                    supabase_client.table("bookings")
                    .select("total_amount, status, nights")
                ...
                total_revenue = sum(
                    float(b["total_amount"]) for b in confirmed_bookings
                )
                ...
                ratings = [
                    float(p["rating"]) for p in properties_result.data if p["rating"]
                ]
                ...
                occupancy_rate = (
                    (total_nights / total_possible_nights * 100)
    ...
    async def optimize_table_indexes():
        ...
        try:
            ...
            logger.info(
                f"Database optimization suggestions: {len(optimization_suggestions)} recommendations"
            )
    ...
    async def cleanup_old_data(days_to_keep: int = 365):
        ...
        async with DatabaseOptimizer.query_timer("cleanup", "old_data"):
            try:
                ...
                analytics_result = (
                    supabase_client.table("analytics_reports")
                    .delete()
                    .lt("created_at", cutoff_date)
                    .execute()
                )
                cleanup_results["analytics_reports"] = (
                    len(analytics_result.data) if analytics_result.data else 0
                )
                ...
                cleanup_results["cancelled_bookings"] = (
                    len(old_cancelled.data) if old_cancelled.data else 0
                )

                logger.info(
                    f"Database cleanup completed", cleanup_results=cleanup_results
                )
