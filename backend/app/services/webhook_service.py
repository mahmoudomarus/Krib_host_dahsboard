...
class WebhookService:
    ...
    async def send_webhook(
        ...
        try:
            ...
            return {
                ...
                "successful_deliveries": len([r for r in results if r.get("status") == "success"])
            }
    ...
    async def _send_single_webhook(
        ...
        async with aiohttp.ClientSession(
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"Sending webhook attempt {attempt + 1} to {subscription['agent_name']}")
                    ...
                    async with session.post(
                        ...
                        if response.status == 200:
                            ...
                            logger.info(f"Webhook delivered successfully to {subscription['agent_name']}")
                            ...
                            return {
                                ...
                                "response_body": response_text[:500]  # Truncate for logging
                            }
                        else:
                            logger.warning(
                    ...
                    logger.error(f"HTTP error on attempt {attempt + 1} to {subscription['agent_name']}: {e}")
                    ...
                    logger.error(f"Unexpected error on attempt {attempt + 1} to {subscription['agent_name']}: {e}")
    ...
    def _generate_signature(self, payload: Dict[str, Any]) -> str:
        ...
        secret_key = getattr(settings, 'webhook_secret_key', 'default_webhook_secret_key')
    ...
    async def _update_webhook_failure(self, subscription_id: str):
        ...
        try:
            ...
            result = supabase_client.table("webhook_subscriptions").select(
                "failed_attempts, max_failed_attempts, agent_name"
            ).eq("id", subscription_id).execute()
    ...
    async def get_webhook_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_result = supabase_client.table("webhook_subscriptions").select("id", count="exact").execute()
            active_result = supabase_client.table("webhook_subscriptions").select("id", count="exact").eq("is_active", True).execute()
            failed_result = supabase_client.table("webhook_subscriptions").select("id", count="exact").eq("is_active", False).execute()
            
            # Get recent successful calls
            from datetime import timedelta
            recent_success_result = supabase_client.table("webhook_subscriptions").select("id", count="exact").gte(
                "last_successful_call", (datetime.utcnow() - timedelta(hours=24)).isoformat()
            ).execute()
            ...
            return {
                ...
                "success_rate": round(
                    (recent_success_result.count or 0) / max(active_result.count or 1, 1) * 100, 2
                ),
