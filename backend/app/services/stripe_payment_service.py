...
class StripePaymentService:
    ...
    async def create_payment_intent(
        ...
        try:
            ...
            if not host_stripe_account:
                raise ValueError(
                    f"Property host does not have a Stripe Connect account set up"
                )
    ...
    async def confirm_payment(
        payment_intent_id: str, booking_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            booking_result = (
                supabase_client.table("bookings")
                .select("id, total_amount, property_id")
                .eq("id", booking_id)
                .execute()
            )
            ...
            platform_fee = total_amount * (
                Decimal(str(StripeConfig.PLATFORM_FEE_PERCENTAGE)) / 100
            )
            ...
            supabase_client.table("bookings").update(update_data).eq(
                "id", booking_id
            ).execute()

            logger.info(
                f"Updated booking {booking_id} with payment status: {payment_intent.status}"
            )
            ...
            return {
                ...
                "paid_at": (
                    datetime.fromtimestamp(payment_intent.created)
                    if payment_intent.status == "succeeded"
                    else None
                ),
            }
    ...
    async def create_refund(
        booking_id: str, amount: Optional[Decimal] = None, reason: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            refund = stripe.Refund.create(
                ...
                metadata={"booking_id": booking_id, "platform": "krib_host_dashboard"},
            )

            logger.info(
                f"Created refund {refund.id} for booking {booking_id}, amount: {refund_amount}"
            )

            # Update booking
            new_refund_total = already_refunded + refund_amount
            new_payment_status = (
                "refunded" if new_refund_total >= total_amount else "partially_refunded"
            )
    ...
    async def handle_payment_intent_succeeded(
        payment_intent_id: str, payment_intent_data: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if not booking_result.data:
                logger.warning(
                    f"No booking found with payment intent {payment_intent_id}"
                )
                return
            ...
            logger.error(
                f"Error handling payment_intent.succeeded for {payment_intent_id}: {e}"
            )
            raise
