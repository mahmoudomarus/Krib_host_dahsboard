...
class AIService:
    def __init__(self):
        ...
        if settings.openai_api_key:
            ...
            logger.warning(
                "OpenAI API key not provided. OpenAI features will be disabled."
            )
        ...
        if settings.anthropic_api_key:
            ...
            logger.warning(
                "Anthropic API key not provided. Anthropic features will be disabled."
            )

    async def generate_property_description(
        self, property_data: Dict[str, Any], use_anthropic: bool = False
    ) -> Dict[str, str]:
        """
    ...
    async def generate_amenities_suggestions(
        self, property_data: Dict[str, Any], existing_amenities: List[str] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            base_amenities = self._get_base_amenities(
            ...
            return self._get_base_amenities(
                property_data.get("property_type", "apartment")
            )[:5]

    async def optimize_listing_title(
        self, original_title: str, property_data: Dict[str, Any]
    ) -> List[str]:
        """
    ...
    async def generate_pricing_strategy(
        self, property_data: Dict[str, Any], market_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.openai_client and market_data:
                ai_insights = await self._get_ai_pricing_insights(
...
Whether you're here for work or play, this thoughtfully designed space offers everything you need for an exceptional stay. Book now and experience the perfect blend of comfort, style, and convenience!"""
    ...
    def _get_space_details(
        self, property_type: str, bedrooms: int, bathrooms: int, max_guests: int
    ) -> str:
    ...
    async def _get_ai_amenities_suggestions(
        self, property_data: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _optimize_title_with_ai(
        self, title: str, property_data: Dict[str, Any]
    ) -> List[str]:
