...
def test_secure_secret_generation(monkeypatch):
    ...
    ), f"secret_key length is {len(settings.secret_key)}, should be >= 32"
    assert (
        len(settings.jwt_secret_key) >= 32
    ), f"jwt_secret_key length is {len(settings.jwt_secret_key)}, should be >= 32"
    assert (
        len(settings.webhook_secret_key) >= 32
    ), f"webhook_secret_key length is {len(settings.webhook_secret_key)}, should be >= 32"
